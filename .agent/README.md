...
**Agents:** after any meaningful change, **append** *why* to `COMMITLOG` (not a file dump). Do not overwrite other WIP lines. Replace the file only when it is a lone `committed <timestamp>` stamp. Follow `SOP.md`. Do not run `commit.sh` unless the human asks — it prompts on a real TTY.
